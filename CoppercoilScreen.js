...
const CoppercoilScreen = () => {
  ...
  useEffect(() => {
    const unsubscribe = db.collection('reviews').onSnapshot((querySnapshot) => {
      ...
      const filteredReviews = reviewsData.filter((review) => review.contraceptiveID === "5iZ62OFb62Z547jo0ryL");
  ...
  return (
    <View style={styles.container}>
      <Image style={styles.contraceptiveImages} source={{ uri: copperCoil?.contraceptiveImage }} />
      ...
      <ScrollView>
        {reviews
          .filter((review) => review.contraceptiveID === "5iZ62OFb62Z547jo0ryL")
          .map((review) => (
