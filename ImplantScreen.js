...
const ImplantScreen = () => {
  ...
  const handleSubmit = (event) => {
    ...
    db.collection('reviews').add({
      contraceptiveID: "6uO4wiEZWJqclbpRiCb6",
  ...
  const implant = contraceptives.find(c => c.contraceptiveName === "Implant");
  const filteredReviews = reviews.filter((review) => review.contraceptiveID === "6uO4wiEZWJqclbpRiCb6");
  const starAverage = filteredReviews.reduce((acc, review) => acc + review.reviewRating, 0) / filteredReviews.length;

  return (
    <View style={styles.container}>
      <Image style={styles.contraceptiveImages} source={{ uri: implant?.contraceptiveImage }} />
      ...
      <ScrollView style={styles.reviewsList}>
        {reviews.filter((review) => review.contraceptiveID === "6uO4wiEZWJqclbpRiCb6").map((review) => (
